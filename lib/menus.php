...
/*
	Register menus
*/
add_action('init', 'cryst4l_menus'); 
function cryst4l_menus(){
    register_nav_menus(array( 
        'header-menu' => __('Header Menu', 'cryst4l'), 
        'sidebar-menu' => __('Sidebar Menu', 'cryst4l'),
        'footer-menu' => __('Footer Menu', 'cryst4l'),
    ));
}
/**
 * Cryst4l Walker Menu
 * Based on https://github.com/roikles/Wordpress-Bem-Menu
 * Inserts some BEM naming sensibility into Wordpress menus
 * Makes use of microdata
 */
class walker_Microdata_BEM extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $output .= $indent . '<li class="' . $class_string . '" itemscope itemtype="http://schema.org/SiteNavigationElement">';
        // link attributes
        $attributes  = ! empty($item->attr_title) ? ' title="'  . esc_attr($item->attr_title) .'"' : '';
        $attributes .= ! empty($item->target)     ? ' target="' . esc_attr($item->target    ) .'"' : '';
        $attributes .= ! empty($item->xfn)        ? ' rel="'    . esc_attr($item->xfn       ) .'"' : '';
        $attributes .= ! empty($item->url)        ? ' href="'   . esc_attr($item->url       ) .'"' : '';
        $item_output = sprintf('%1$s<a%2$s itemprop="url"><span itemprop="name">%3$s%4$s%5$s</span></a>%6$s', $args->before, $attributes, $args->link_before, apply_filters('the_title', $item->title, $item->ID), $args->link_after, $args->after);
        // Filter <li>
...
/**
 * cryst4l_menu returns an instance of the walker_Microdata_BEM class with the following arguments
 ...
 * @return [type]
 */
function cryst4l_menu($location = "main_menu", $css_class_prefix = 'main-menu', $css_class_modifiers = null){  
    ...
    $args = array(
        ...
        'walker'            => new walker_Microdata_BEM($css_class_prefix, true)
    );
